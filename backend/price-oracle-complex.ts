...
export class PriceOracle {
  ...
  private async fetchChainlinkPrice(
    ...
    try {
      // Import Chainlink oracle dynamically to avoid circular dependencies
      const { chainlinkOracle } = await import('./oracles/chainlink-oracle');

      const result = await chainlinkOracle.getPrice(token.symbol, chain);

      return {
        priceUsd: result.priceUsd,
        source: "chainlink",
        timestamp: result.timestamp,
        confidence: 0.95, // Chainlink feeds have high confidence
        metadata: {
          blockNumber: undefined, // Not available from Chainlink
          rawData: {
            roundId: result.roundId.toString(),
            feedAddress: result.feedAddress,
            ageSeconds: result.ageSeconds,
          },
        },
      };
    } catch (error) {
      log(`Chainlink oracle error: ${error}`, 'price-oracle');
      return null;
    }
  }
  ...
  private async fetchUniswapV3TWAP(
    ...
    try {
      // Import Uniswap TWAP oracle dynamically
      const { uniswapTwapOracle } = await import('./oracles/uniswap-twap-oracle');

      const result = await uniswapTwapOracle.getPrice(token.symbol, chain);

      return {
        priceUsd: result.priceUsd,
        source: "uniswap-v3-twap",
        timestamp: result.timestamp,
        confidence: 0.85, // TWAP has slightly lower confidence than Chainlink
        metadata: {
          blockNumber: undefined,
          twapPeriod: result.observationWindowSeconds,
          rawData: {
            poolAddress: result.poolAddress,
            twapTick: result.twapTick,
            spotTick: result.spotTick,
            deviationFromSpot: result.deviationFromSpot,
            liquidity: result.liquidity.toString(),
          },
        },
      };
    } catch (error) {
      log(`Uniswap TWAP error: ${error}`, 'price-oracle');
      return null;
    }
  }
  ...
  private async fetchCoingeckoPrice(
    ...
    try {
      // CoinGecko Simple Price API (free tier, rate limited)
      const url = `https://api.coingecko.com/api/v3/simple/price?ids=${token.priceFeeds.coingeckoId}&vs_currencies=usd`;
      ...
      if (!response.ok) {
        log(`CoinGecko API error: ${response.status} ${response.statusText}`, 'price-oracle');
        return null;
      }

      const data = await response.json();
      const priceUsd = data[token.priceFeeds.coingeckoId]?.usd;

      if (typeof priceUsd !== 'number' || priceUsd <= 0) {
        log(`Invalid CoinGecko price data for ${token.symbol}`, 'price-oracle');
        return null;
      }
      ...
      return {
        ...
        source: "coingecko",
        timestamp: new Date(),
        confidence: 0.75, // CoinGecko has moderate confidence (CEX aggregation)
        metadata: {
          rawData: { coingeckoId: token.priceFeeds.coingeckoId },
        },
      };
    } catch (error) {
      // Log error but don't throw - this is a fallback source
      log(`CoinGecko API error: ${error}`, 'price-oracle');
      return null;
    }
  }
