...
-- Adds unique constraint to prevent duplicate purchases at database level
-- This prevents race conditions where two simultaneous requests could both pass checks

-- =====================================================
-- 1. UNIQUE CONSTRAINT FOR COMPLETED PURCHASES
-- Prevents duplicate purchases for the same prompt by the same buyer
-- =====================================================

-- Create unique partial index for completed purchases only
-- This allows multiple pending purchases but only one completed purchase per prompt/buyer
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_completed_purchase 
ON prompt_purchases (prompt_id, buyer_id) 
WHERE status = 'completed';

-- Add comment
COMMENT ON INDEX idx_unique_completed_purchase IS 
'Prevents duplicate completed purchases. Allows multiple pending purchases but only one completed purchase per prompt per buyer.';

-- =====================================================
-- 2. UPDATE ATOMIC FUNCTION TO USE ON CONFLICT
-- Modify the atomic purchase function to handle conflicts gracefully
-- =====================================================

-- Note: The function already checks for duplicates, but we'll enhance it
-- to use INSERT ... ON CONFLICT for better race condition handling
...
'Atomically records a prompt purchase and updates creator earnings. Prevents duplicate purchases using unique constraint and ON CONFLICT handling.';